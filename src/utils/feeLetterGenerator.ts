...
export const generateFeeLetterText = (data: IFeeLetterData) => {
  ...
  output += `<p className='text-indent'>1. \tJasmeen</p>\n\n`;
  output += `<p class='fee-list-item'>1. \tGoyani <b>test...</b></p>\n\n`;
  // Introduction
  output += `<p className='text-indent'>1. \tWe refer to the Facility Agreement.</p>\n\n`;
  output += `<p className='text-indent'>2. \tUnless otherwise defined in this Fee Letter, terms defined in the Facility Agreement shall have the same meaning in this Fee Letter. The rules of interpretation and construction contained in clause *{[1.2] (Construction)}* of the Facility Agreement shall apply to this Fee Letter as if set out in full in this Fee Letter mutatis mutandis, and references in those clauses to "this Agreement" or the *{Finance Documents}* shall be construed as references to this Fee Letter.</p>\n\n`;

  // Specific introduction based on letter type
  switch (data.letterType) {
    case 'facilityAgent':
      output += `<p>3. \tThis Letter is the Facility Agent Fee Letter.</p>\n\n`;
      break;
    case 'securityAgent':
      output += `<p>3. \tThis Letter is the Security Agent Fee Letter.</p>\n\n`;
      break;
    case 'globalCoordinator':
      output += `<p>3. \tThis Letter is the Global Co-Ordinator Fee Letter.</p>\n\n`;
      break;
    case 'coordinatingBank':
      output += `<p>3. \tThis Letter is the Co-Ordinating Bank Fee Letter.</p>\n\n`;
      break;
    case 'upfront':
      output += `<p>3. \tThis Letter is the Upfront Fee Letter.</p>\n\n`;
      break;
    case 'mandatedLeadArranger':
      output += `<p>3. \tThis Letter is the Mandated Lead Arranger Fee Letter.</p>\n\n`;
      break;
    case 'arranger':
      output += `<p>3. \tThis Letter is the Arranger Fee Letter.</p>\n\n`;
      break;
    default:
      output += `<p>3. \tThis Letter is the Facility Agent Fee Letter.</p>\n\n`;
  }
  ...
  output += `<p>4. \tThis ${getLetterTitle()
    .replace(/\(|\)/g, '')
    .toLowerCase()
    .split(' ')
  ...
  switch (data.letterType) {
    case 'facilityAgent':
      if (data.hasFacilityAgentOptions) {
        output += `<p>5. You shall pay to the Facility Agent (for its own account):</p>\n\n`;
        output += `<ul class="fee-sublist">\n`;

        if (data.setupFeeAmount) {
          output += `<li class="fee-sublist-item">(a) a one-off set-up fee in an amount of ${data.currency} ${data.setupFeeAmount} (the <b>"Set-Up Fee"</b>),</li>\n\n`;
        }

        output += `<li class="fee-sublist-item">(b) a fee of ${data.currency}${data.facilityAgentFeeAmount} per annum, which is payable in advance, for the Facility Agent acting as facility agent pursuant to the provisions of the Facility Agreement (the <b>"Facility Agent Fee"</b>),</li>\n\n`;

        if (data.increaseFeeAmount) {
          output += `<li class="fee-sublist-item">(c) a fee of ${data.currency} ${data.increaseFeeAmount} if the Borrower exercises the Increase Option more than ${data.increaseCount} times (the "Increase Fee"), and</li>\n\n`;
        }

        if (data.debtdomainFeeAmount) {
          output += `<li class="fee-sublist-item">(d) a fee of ${data.currency}${data.debtdomainFeeAmount} per annum, which is payable in advance, for the use of a publishing system known as Debtdomain to assist in certain administrative tasks specific to the Facility Agent under the terms of the Facility Agreement (the <b>"Debtdomain Fee"</b>). The Facility Agent reserves the right to increase the Debtdomain Fee at its discretion if Debtdomain charges increase provided always that such increase of the Debtdomain Fee shall be more than the increase of Debtdomain charges.</li>\n\n`;
        }

        output += `<li class="fee-sublist-item">(e) The Set-Up Fee, Facility Agent Fee, Increase Fee and Debtdomain Fee shall hereinafter be referred to as the <b>"Agency Fees"</b></li>\n`;
        output += `</ul>\n\n`;
      } else {
    ...
    case 'securityAgent':
      output += `<p>5. You shall pay to the Security Agent (for its own account) a fee of ${data.currency} ${data.securityAgentFeeAmount} per annum, which is payable in advance, for the Security Agent acting as security agent pursuant to the provisions of the Facility Agreement (the <b>"Security Agent Fee"</b>).\n\n`;
      break;

    case 'globalCoordinator':
      output += `<p>5. You shall pay to the Global Co-Ordinator (for its own account) a flat fee of ${data.currency} ${data.globalCoordinatorFeeAmount} for the Global Co-Ordinator acting as global co-ordinator pursuant to the provisions of the Facility Agreement (the <b>"Global Co-Ordinator Bank Fee"</b>).\n\n`;
      break;

    case 'coordinatingBank':
      output += `<p>5. You shall pay to the Co-Ordinating Bank (for its own account) a flat fee of ${data.currency}${data.coordinatingBankFeeAmount} for the CoOrdinating Bank acting as co-ordinating bank pursuant to the provisions of the Facility Agreement (the <b>"Coordinating Bank Fee"</b>).</p>\n\n`;
      break;

    case 'upfront':
      output += `<p>5. You shall pay to the Facility Agent (for the account of the *{Original Lenders}*) a fee of ${data.currency} ${data.upfrontFeeAmount} for the *{Original Lenders}* extending their Commitments to the Borrower under the Facility Agreement (an "Upfront Fee"). Each *{Original Lender}* shall be entitled to its pro rata share of the Upfront Fee. Its pro rata share shall be equal to the proportion its Commitment bears to the Total Commitment.</p>\n\n`;
      break;
    ...
    default:
      ...
      output += `<p>Only one type: facility agent pursuant to the provisions of the Facility Agreement (the <b>"Facility Agent Fee"</b>).</p>\n\n`;
  }
  ...
  switch (data.letterType) {
    case 'facilityAgent':
      if (data.hasFacilityAgentOptions) {
        ...
        if (data.paymentModality === 'annual') {
          output += `<li class="fee-sublist-item">(b) The first payment of the Facility Agent Fee is payable on or before [the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement (the "FirstPayment Date")][the First Payment Date]. Each subsequent payment of the Facility Agent Fee shall be made annually in advance, commencing on the first anniversary of the First Payment Date and thereafter on each subsequent anniversary of the First Payment Date, until (but excluding) the earlier of</li>\n
          <ul>
          <li class='fee-sublist-level2'>(i) the *{Final Maturity Date}*, </li>\n
          <li class='fee-sublist-level2'>(ii) the date we resign or are replaced as facility agent and  </li>\n
          <li class='fee-sublist-level2'>(iii) the date of prepayment of all *{Loans}* and cancellation - of all *{Commitments}*, in each case under the Facility Agreement (the <b>"Final Payment Date"</b>). </li></ul>\n`;
        ...
        output += `<li class="fee-sublist-item">(c) The Increase Fee shall be payable to the Facility Agent on or prior to the date falling {[${data.businessDays}]} *{Business Days}* after the Increase Date and on or prior to the date falling {[${data.businessDays}]} *{Business Days}* after each subsequent Increase Date.</li>\n`;
        output += `<li class="fee-sublist-item">(d) The first payment of the Debtdomain Fee is payable on or before the First Payment Date. Each subsequent payment of the Debtdomain Fee shall be made annually in advance, commencing on the first anniversary of the First Payment Date and thereafter on each subsequent anniversary of the First Payment Date, until the Final Payment Date..</li>\n\n`;
      }
      break;
    ...
    case 'globalCoordinator':
      output += `<p>Payment Modalities Global Co-Ordinating Bank\n: </p><p>6. The Global Co-ordinator Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</p>\n\n`;
      break;

    case 'coordinatingBank':
      output += `<p>Payment Modalities Co-Ordinating Bank\n: </p><p>6. The Co-ordinating Bank Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</p>\n\n`;
      break;

    case 'upfront':
      output += `<p>Payment Modalities Upfront\n: </p><p>6. The Upfront Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</p>\n\n`;
      break;

    case 'mandatedLeadArranger':
      output += `<p>Payment Modalities Mandate Lead Arranger\n: </p><p>6. The Mandated Lead Arranger Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</p>\n\n`;
      break;

    case 'arranger':
      output += `<p>Payment Modalities Arranger\n: </p><p>6. The Arranger Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</p>\n\n`;
      break;
  ...
  if (data.bankDetails.accountBank || data.bankDetails.accountNumber) {
    output += `<p>7. All payments made by you under this Fee Letter must be made in immediately available, freely transferable and cleared funds by crediting the following bank account \nAccount Bank: ${data.bankDetails.accountBank}\nAccount Holder: ${data.bankDetails.accountHolder}\nAccount No.: ${data.bankDetails.accountNumber}\nSort Code: ${data.bankDetails.sortCode}\nIBAN: ${data.bankDetails.iban}\nRef: ${data.borrowerName}-${data.currency} ${data.amount} ${data.facilityType} Facility Agreement\nor such other account as we may notify you in writing with {[${data.businessDays}]} *{Business Days}* prior notice.</p>\n\n`;
  } else {
    output += `<p>7. A11 payments made by you under this Fee Letter must be made in immediately available, freely transferable and cleared funds by crediting our account (the details of which we have provided you with separately) or such other account as we may notify you in writing with {[${data.businessDays}]} *{Business Days'}*' prior notice.</p>\n\n`;
  }
  ...
  if (data.grossUpProvision === 'Standalone') {
    output += `<p>8. Clause *{ {12} (Tax Gross Up and Indemnities)}* of the Facility Agreement shall apply to this letter as if set out in full in this letter, mutatis mutandis, and references in that clause to Finance Documents shall be construed as including this Fee Letter.</p>\n\n`;
  } else {
    output += `<p>8. If any *{Tax}* or amount in respect of a *{Tax}* must be deducted or withheld from any amount payable or paid by the ${data.borrowerName} on account of or by reference to any payment by or obligation of the ${data.borrowerName} hereunder, the ${data.borrowerName} shall pay such additional amounts as may be necessary to ensure that the recipient of such payment receives a net amount equal to the full amount which we would have received from the ${data.borrowerName} had such *{Tax}* not been made.</p>
\n\n`;
  }

  //For VAT Provision
  if (data.vatProvision === 'Standalone') {
    output += `<p>9. Clause *{ {¢} (VAT)}* of the Facility Agreement shall apply to this letter as if set out in full in this letter, mutatis mutandis, and references in that clause to Finance Documents shall be construed as including this Fee Letter.</p>\n\n`;
  } else {
    output += `<p>9. All payments in connection with this Fee Letter are exclusive of any value added tax or similar charge ("<b>VAT</b>"). If VAT is chargeable, the ${data.borrowerName} shall also and at the same time pay to the recipient of the relevant payment an amount equal to the amount of the VAT.</p>\n\n`;
  }
  ...
  // Miscellaneous
  output += `<p>11. This Fee Letter and your rights hereunder may not be assigned by you without our prior written consent.</p>\n\n`;
  ...
  output += `<p>12. This ${agentName} Fee Letter may not be amended or waived except as may be agreed in writing by the ${agentName} and the Borrower.</p>\n\n`;
  output += `<p>13. This ${agentName} Fee Letter may be signed in any number of counterparts. This has the same effect as if the signatures on the counterparts were on a single copy of this Fee Letter.</p>\n\n`;
  output += `<p>14. A person who is not a party to this Fee Letter shall have no rights under the Contracts (Rights of Third Parties) Act 1999 or otherwise to enforce or enjoy the benefit of any terms of this Fee Letter.</p>\n\n`;
  output += `<p>15. This letter and all non-contractual obligations arising from or connected with it are governed by ${data.governingLaw} law.</p>\n\n`;
  output += `<p>16. Clause {¢} (Enforcement) of the Facility Agreement shall apply to this Fee Letter as if set out in full in this Fee Letter mutatis mutandis, and references in those clauses to "this Agreement" or the Finance Documents shall be construed as references to this Fee Letter.</p>\n\n`;
  ...
  output += `<p>Yours faithfully</p>\n\n\n\n`;
  output += `<p>_______________</p>\n\n`;
  output += `<p>For and on behalf of \n<b>${getSenderName()}</b></p>\n\n\n`;
  output += `<p>Agreed and accepted by</p>\n\n`;
  output += `<p>_______________</p>\n\n`;
  output += `<p>For and on behalf of \n<b>${data.borrowerName}</b></p>\n`;
