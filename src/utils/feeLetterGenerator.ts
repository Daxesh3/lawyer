...
export const generateFeeLetterText = (data: IFeeLetterData) => {
  ...
  // Introduction
  output += `<div class='flex'><p class='w-4 mr-2'>1. </p>We refer to the Facility Agreement.</div>\n\n`;
  output += `<div class='flex'><p class='w-4 mr-2'>2. </p>Unless otherwise defined in this Fee Letter, terms defined in the Facility Agreement shall have the same meaning in this Fee Letter. The rules of interpretation and construction contained in clause *{[1.2] (Construction)}* of the Facility Agreement shall apply to this Fee Letter as if set out in full in this Fee Letter mutatis mutandis, and references in those clauses to "this Agreement" or the *{Finance Documents}* shall be construed as references to this Fee Letter.</div>\n\n`;

  // Specific introduction based on letter type
  switch (data.letterType) {
    case 'facilityAgent':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Facility Agent Fee Letter.</div>\n\n`;
      break;
    case 'securityAgent':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Security Agent Fee Letter.</div>\n\n`;
      break;
    case 'globalCoordinator':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Global Co-Ordinator Fee Letter.</div>\n\n`;
      break;
    case 'coordinatingBank':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Co-Ordinating Bank Fee Letter.</div>\n\n`;
      break;
    case 'upfront':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Upfront Fee Letter.</div>\n\n`;
      break;
    case 'mandatedLeadArranger':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Mandated Lead Arranger Fee Letter.</div>\n\n`;
      break;
    case 'arranger':
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Arranger Fee Letter.</div>\n\n`;
      break;
    default:
      output += `<div class='flex'><p class='w-4 mr-2'>3. </p>This Letter is the Facility Agent Fee Letter.</div>\n\n`;
  }
  ...
  output += `<div class='flex'><p class='w-4 mr-2'>4. </p>This ${getLetterTitle()
    .replace(/\(|\)/g, '')
    .toLowerCase()
    .split(' ')
  ...
  switch (data.letterType) {
    case 'facilityAgent':
      if (data.hasFacilityAgentOptions) {
        output += `<div class='flex'><p class='w-4 mr-2'>5. </p>You shall pay to the Facility Agent (for its own account):</div>\n\n`;

        if (data.setupFeeAmount) {
          output += `<div class='flex'><p class='ml-8 mr-2'>(a) </p><p>a one-off set-up fee in an amount of ${data.currency} ${data.setupFeeAmount} (the <b>"Set-Up Fee"</b>),</p></div>\n\n`;
        }

        output += `<div class='flex'><p class='ml-8 mr-2'>(b) </p><p>a fee of ${data.currency}${data.facilityAgentFeeAmount} per annum, which is payable in advance, for the Facility Agent acting as facility agent pursuant to the provisions of the Facility Agreement (the <b>"Facility Agent Fee"</b>),</p></div>\n\n`;

        if (data.increaseFeeAmount) {
          output += `<div class='flex'><p class='ml-8 mr-2'>(c) </p><p>a fee of ${data.currency} ${data.increaseFeeAmount} if the Borrower exercises the Increase Option more than ${data.increaseCount} times (the "Increase Fee"), and</p></div>\n\n`;
        }

        if (data.debtdomainFeeAmount) {
          output += `<div class='flex'><p class='ml-8 mr-2'>(d) </p><p><p>fee of ${data.currency}${data.debtdomainFeeAmount} per annum, which is payable in advance, for the use of a publishing system known as Debtdomain to assist in certain administrative tasks specific to the Facility Agent under the terms of the Facility Agreement(the  <b>"Debtdomain Fee"</b>). The Facility Agent reserves the right to increase the Debtdomain Fee at its discretion if Debtdomain charges increase provided always that such increase of the Debtdomain Fee shall be more than the increase of Debtdomain charges.</p></div>\n\n`;
        }

        output += `<div class='flex'><p class='ml-8 mr-2'>(e) </p><p>The Set-Up Fee, Facility Agent Fee, Increase Fee and Debtdomain Fee shall hereinafter be referred to as the <b>"Agency Fees"</b>.</p></div>\n\n`;
      } else {
        output += `<div class='flex'><p class='ml-8 mr-2'>Only one type: facility agent pursuant to the provisions of the Facility Agreement (the <b>"Facility Agent Fee"</b>).</p></div>\n\n`;
      }
      break;

    case 'securityAgent':
      output += `<div class='flex'><p class='w-4 mr-2'>5. </p><p>You shall pay to the Security Agent (for its own account) a fee of ${data.currency} ${data.securityAgentFeeAmount} per annum, which is payable in advance, for the Security Agent acting as security agent pursuant to the provisions of the Facility Agreement (the <b>"Security Agent Fee"</b>).</p></div>\n\n`;
      break;

    case 'globalCoordinator':
      output += `<div class='flex'><p class='w-4 mr-2'>5. </p><p>You shall pay to the Global Co-Ordinator (for its own account) a flat fee of ${data.currency} ${data.globalCoordinatorFeeAmount} for the Global Co-Ordinator acting as global co-ordinator pursuant to the provisions of the Facility Agreement (the <b>"Global Co-Ordinator Bank Fee"</b>).</p></div>\n\n`;
      break;

    case 'coordinatingBank':
      output += `<div class='flex'><p class='w-4 mr-2'>5. </p><p>You shall pay to the Co-Ordinating Bank (for its own account) a flat fee of ${data.currency}${data.coordinatingBankFeeAmount} for the CoOrdinating Bank acting as co-ordinating bank pursuant to the provisions of the Facility Agreement (the <b>"Coordinating Bank Fee"</b>).</p></div>\n\n`;
      break;

    case 'upfront':
      output += `<div class='flex'><p class='w-4 mr-2'>5. </p>You shall pay to the Facility Agent (for the account of the *{Original Lenders}*) a fee of ${data.currency} ${data.upfrontFeeAmount} for the *{Original Lenders}* extending their Commitments to the Borrower under the Facility Agreement (an "Upfront Fee"). Each *{Original Lender}* shall be entitled to its pro rata share of the Upfront Fee. Its pro rata share shall be equal to the proportion its Commitment bears to the Total Commitment.</div>\n\n`;
      break;
  ...
  switch (data.letterType) {
    case 'facilityAgent':
      if (data.hasFacilityAgentOptions) {
        ...
        output += `<div class='flex'><p class='ml-8 mr-2'>(c) </p><p>The Increase Fee shall be payable to the Facility Agent on or prior to the date falling {[${data.businessDays}]} *{Business Days}* after the Increase Date and on or prior to the date falling {[${data.businessDays}]} *{Business Days}* after each subsequent Increase Date.</p></div>\n`;
        output += `<div class='flex'><p class='ml-8 mr-2'>(d) </p><p>The first payment of the Debtdomain Fee is payable on or before the First Payment Date. Each subsequent payment of the Debtdomain Fee shall be made annually in advance, commencing on the first anniversary of the First Payment Date and thereafter on each subsequent anniversary of the First Payment Date, until the Final Payment Date.</p></div>\n\n`;
      }
      break;
    ...
    case 'globalCoordinator':
      output += `Payment Modalities Global Co-Ordinating Bank\n:<div class='flex'><p class='w-4 mr-2'>6.</p> The Global Co-ordinator Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</div>\n\n`;
      break;

    case 'coordinatingBank':
      output += `Payment Modalities Co-Ordinating Bank\n:<div class='flex'><p class='w-4 mr-2'>6.</p> The Co-ordinating Bank Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</div>\n\n`;
      break;

    case 'upfront':
      output += `Payment Modalities Upfront\n:<div class='flex'><p class='w-4 mr-2'>6.</p> The Upfront Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</div>\n\n`;
      break;

    case 'mandatedLeadArranger':
      output += `Payment Modalities Mandate Lead Arranger\n:<div class='flex'><p class='w-4 mr-2'>6.</p> The Mandated Lead Arranger Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</div>\n\n`;
      break;

    case 'arranger':
      output += `Payment Modalities Arranger\n:<div class='flex'><p class='w-4 mr-2'>6.</p> The Arranger Fee is payable on or before the earlier to occur of the date falling {[${data.businessDays}]} *{Business Days}* after the date of the Facility Agreement and the first *{Utilisation Date}* under the Facility Agreement.</div>\n\n`;
      break;
  ...
  if (data.bankDetails.accountBank || data.bankDetails.accountNumber) {
    output += `<div class='flex'><p class='w-4 mr-2'>7. </p> All payments made by you under this Fee Letter must be made in immediately available, freely transferable and cleared funds by crediting the following bank account \nAccount Bank: ${data.bankDetails.accountBank}\nAccount Holder: ${data.bankDetails.accountHolder}\nAccount No.: ${data.bankDetails.accountNumber}\nSort Code: ${data.bankDetails.sortCode}\nIBAN: ${data.bankDetails.iban}\nRef: ${data.borrowerName}-${data.currency} ${data.amount} ${data.facilityType} Facility Agreement\nor such other account as we may notify you in writing with {[${data.businessDays}]} *{Business Days}* prior notice.</div>\n\n`;
  } else {
    output += `<div class='flex'><p class='w-4 mr-2'>7. </p> A11 payments made by you under this Fee Letter must be made in immediately available, freely transferable and cleared funds by crediting our account (the details of which we have provided you with separately) or such other account as we may notify you in writing with {[${data.businessDays}]} *{Business Days'}*' prior notice.</div>\n\n`;
  }
  ...
  if (data.grossUpProvision === 'Standalone') {
    output += `<div class='flex'><p class='w-4 mr-2'>8. </p> Clause *{ {12} (Tax Gross Up and Indemnities)}* of the Facility Agreement shall apply to this letter as if set out in full in this letter, mutatis mutandis, and references in that clause to Finance Documents shall be construed as including this Fee Letter.</div>\n\n`;
  } else {
    output += `<div class='flex'><p class='w-4 mr-2'>8. </p> If any *{Tax}* or amount in respect of a *{Tax}* must be deducted or withheld from any amount payable or paid by the ${data.borrowerName} on account of or by reference to any payment by or obligation of the ${data.borrowerName} hereunder, the ${data.borrowerName} shall pay such additional amounts as may be necessary to ensure that the recipient of such payment receives a net amount equal to the full amount which we would have received from the ${data.borrowerName} had such *{Tax}* not been made.</div>
\n\n`;
  }

  //For VAT Provision
  if (data.vatProvision === 'Standalone') {
    output += `<div class='flex'><p class='w-4 mr-2'>9. </p> Clause *{ {¢} (VAT)}* of the Facility Agreement shall apply to this letter as if set out in full in this letter, mutatis mutandis, and references in that clause to Finance Documents shall be construed as including this Fee Letter.</div>\n\n`;
  } else {
    output += `<div class='flex'><p class='w-4 mr-2'>9. </p><p>All payments in connection with this Fee Letter are exclusive of any value added tax or similar charge ("<b>VAT</b>"). If VAT is chargeable, the ${data.borrowerName} shall also and at the same time pay to the recipient of the relevant payment an amount equal to the amount of the VAT.</p></div>\n\n`;
  }

  // Fee refunds - only for facility agent and security agent
  if (data.letterType === 'facilityAgent' || data.letterType === 'securityAgent') {
    output += `<div class='flex'><p class='w-4 mr-2'>10. </p> The ${
  ...
  // Miscellaneous
  output += `<div class='flex'><p class='w-4 mr-2'>11. </p> This Fee Letter and your rights hereunder may not be assigned by you without our prior written consent.</div>\n\n`;
  ...
  output += `<div class='flex'><p class='w-4 mr-2'>12. </p> This ${agentName} Fee Letter may not be amended or waived except as may be agreed in writing by the ${agentName} and the Borrower.</div>\n\n`;
  output += `<div class='flex'><p class='w-4 mr-2'>13. </p> This ${agentName} Fee Letter may be signed in any number of counterparts. This has the same effect as if the signatures on the counterparts were on a single copy of this Fee Letter.</div>\n\n`;
  output += `<div class='flex'><p class='w-4 mr-2'>14. </p> A person who is not a party to this Fee Letter shall have no rights under the Contracts (Rights of Third Parties) Act 1999 or otherwise to enforce or enjoy the benefit of any terms of this Fee Letter.</div>\n\n`;
  output += `<div class='flex'><p class='w-4 mr-2'>15. </p> This letter and all non-contractual obligations arising from or connected with it are governed by ${data.governingLaw} law.</div>\n\n`;
  output += `<div class='flex'><p class='w-4 mr-2'>16. </p> Clause {¢} (Enforcement) of the Facility Agreement shall apply to this Fee Letter as if set out in full in this Fee Letter mutatis mutandis, and references in those clauses to "this Agreement" or the Finance Documents shall be construed as references to this Fee Letter.</div>\n\n`;
